...
import { Loader2, Pencil, Eye, Check } from 'lucide-react';
...
function EditForm({ onClose, clienteData, isAdmin }: Omit<Props, 'isOpen'>) {
    ...
    // Derive header props so Modal gets the right icon + title
    const modalIcon = isAdmin
        ? <Pencil size={16} className="text-orange-400" />
        : <Eye size={16} className="text-blue-400" />;
    const modalTitle = isAdmin ? 'Editar Cliente' : 'Ficha do Cliente';
    const modalSubtitle = isAdmin
        ? 'Dados cadastrais, localização e operação'
    ...
    return (
        ...
        >
            ...
            <div className="overflow-y-auto max-h-[65vh] flex flex-col gap-6 pr-1">

                {/* Non-admin notice banner */}
                {!isAdmin && (
                    <div className="flex items-center gap-2 rounded-xl bg-blue-500/10 border border-blue-500/20 px-4 py-3">
